...
|7. O administrativo escolhe uma competência técnica da lista.  		 ||||
| 8. **O sistema mostra os graus de proficiência aplicáveis a essa competência técnica e pede para selecionar o grau mínimo exigível bem como o seu carácter (i.e. obrigatória ou desejável).**|							 |             |                              |
| 9. **O administrador seleciona o grau pretendido e introduz carácter da competência técnica.** |	... guarda a competência técnica selecionada?   | CaraterCT       | IE: no MD cada Categoria tem várias CaraterCT sendo cada uma referente a uma CompetenciaTecnica (CT).    |
||...cria instância de CaracterCT|Categoria|IE: no MD cada Categoria tem várias CaraterCT.|
||...cria instância de GrauProficiencia|Categoria|IE: no MD cada Categoria tem vários GrauProficiencia.|
| 10. Os passos 7 a 9 repetem-se enquanto não forem introduzidas todas as competências técnicas pretendidas.||||                          
...
|	 |	...valida os dados da Categoria (validação global) | RegistaCategoria  | IE: RegistaCategoria gera Categoria.  |
| 12. O administrativo confirma.   		 |							 |             |                              |
